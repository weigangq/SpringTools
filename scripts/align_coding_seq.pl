...
GetOptions (\%opts, "help|h",
					"man|m",
					"debug",
					"fasta-alignment|a") or pod2usage(2);

pod2usage(1) if $opts{"help"};
pod2usage(-exitstatus => 0, -verbose => 2) if $opts{"man"};

pod2usage(2) if (@ARGV < 2); # Show a short usage statement if missing required arguments
my ($pro, $cds) = @ARGV;

my $cds_in = Bio::SeqIO->new(-file => "<$cds" , '-format' => 'Fasta')
			|| die "Error opening file $cds.\n";
my %seqs; # Will be a hash of display_id=>Bio::Seq object key/value pairs (needed for aa_to_dna_aln)
...
$cds_fasaln_out = Bio::AlignIO->new(-file => ">$pro" . "cds.fas", '-format' => 'Fasta')
			|| die "Error opening file ${pro}.fas for writing.\n";
}

while ( my $aln = $aa_in->next_aln ) {
	my $dna_align = aa_to_dna_aln($aln, \%seqs);
	$dna_align->set_displayname_flat(); # Used in example: removes start/end info
